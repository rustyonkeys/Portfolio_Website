...
export default function Projects() {
  ...
  return (
    <section
      ref={ref}
      id="projects"
      className="w-full min-h-screen flex flex-col justify-between bg-[#1e1e1e] px-6 py-12 text-white"
    >
      ...
      <div className="flex-1 flex flex-col gap-10">
        {filteredProjects.map((project, idx) => (
          <div
            key={idx}
            className="bg-[#2c2c2c] rounded-2xl shadow-md flex flex-col md:flex-row items-center md:items-stretch p-6 md:p-10"
          >
            {/* Left: Description */}
            <div className="flex-1 flex flex-col justify-between md:pr-12">
              <div>
                <h2 className="text-5xl font-bebas font-bold text-orange-400 mb-6">
                  {project.title}
                </h2>
                <p className="text-2xl font-istok text-gray-300 mb-8">
                  {project.description}
                </p>
              </div>
              <a
                ...
                className="mt-auto inline-block text-orange-300 font-semibold transition-colors text-lg flex items-center gap-2 hover:text-orange-500"
              >
                View Project <span className="text-orange-300 ">&#8594;</span>
              </a>
            </div>
            {/* Right: Image */}
            <div className="flex-shrink-0 w-full md:w-[40vw] flex justify-center items-center mt-8 md:mt-0">
              <img
                src={project.image}
                alt={project.title}
                className="rounded-xl shadow-lg object-cover w-full h-[350px] md:h-[500px] bg-gray-800 transition-transform duration-300 hover:scale-105"
              />
            </div>
          </div>
        ))}
